...
class IndustryRepositoryImpl(private val networkClient: NetworkClient) : IndustryRepository {
    override suspend fun getIndustries(): Resource<List<ru.practicum.android.diploma.domain.models.Industry>> {
        val response = networkClient.doIndustryRequest()
        return when (response.resultCode) {
            RESULT_CODE_SUCCESS -> {
                val industryResponse = response as FilterIndustry
                Resource.Success(
                    industryResponse.industries.map {
